...
fn main() {
    ...
    error[E0308]: mismatched types
      --> chapter05/src/bin/casting.rs:11:23
       |
    11 |     let integer: u8 = decimal;
    ...
    //This converts using ascii table
    // println!("Ascii value of i is: {}", 'i' as u8);
    let character = integer as char;

    //There are limitations in conversion rules
    // e.g. a float cannot directly converted into a char
    // let character = decimal as char;
    // FIXME ^ comment out this line
    /*
     error[E0604]: only `u8` can be cast as `char`, not `f32`
      --> chapter05/src/bin/casting.rs:31:21
       |
    31 |     let character = decimal as char;
       |                     ^^^^^^^^^^^^^^^ invalid cast
       |
    */
